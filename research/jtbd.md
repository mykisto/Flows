...
**Evidence legend.** `[N=30]` = beachhead survey (directional, §6). `[desk]` = competitor synthesis / Bogleheads (§1, §5). `[?]` = not in the data.
...
From **P1 (Excel Consolidator)**. Evidence: research goal (the success hypothesis is this exact task); Excel is the biggest incumbent `[N=30, Q4: 40%]`; fragmentation is real `[N=30, Q2: 3+ platforms for 70%]`; income is already tracked by most `[N=30, Q7: 87%]`; the felt pain is the manual gathering `[N=30, R7]`.
...
P1. Evidence: `[N=30, R7]` ("the pain is logging in here and there to gather all the data"); fragmentation `[N=30, Q2]`.
...
P1. Evidence: flexibility is a named want `[N=30, R8]`; no app covers the core assets as anything but manual `[§5 gap #2]`; the open escape hatch is what turns ~90% coverage into 100% `[benchmark §2; desk]`.
...
P1. Evidence: forward + realized is the largest group `[N=30, Q7: 53% both, 33% realized]`.
...
P1. Evidence: Excel is where they consolidate now `[N=30, Q4: 40%]`; switching cost from the sheet is the barrier `[research goal; desk]`.
...
P3 (Trust-Wary Abandoner). Evidence: a third have quit an app over data demands `[N=30, Q8: 37%]`; auto-sync reads as a "red flag" `[Bogleheads, §1]`; but half are comfortable with bank/broker apps, so this is a choice, not a default `[N=30, Q8: 50%]`.
...
P1 / P2. Evidence: scattered assets is the single most-named difficulty `[N=30, Q9: 27%]`.
...
P3. Evidence: real past abandonment over data demands `[N=30, Q8: 37%]`; "red flag" `[Bogleheads]`. Honest caveat: felt as an entry filter, not a daily pain `[N=30, Q9: distrust 0%]`.
...
P1. Evidence: most track income, and half track realized income specifically `[N=30, Q7: 87% track, 33% realized-only + 53% both]`.
...
- **HJ2 (social) - Share my approach.** *When I find a setup that works, I want to compare or share it with the community, so I can help and be helped.* `[?]` Weak signal only: some respondents built and shared their own tools (PowerBI, Notion) `[N=30, §6]`.
- **HJ3 - Track progress toward a long-term goal.** *When I am building toward a goal like financial independence, I want to see whether I am on track, so I can adjust.* `[thin]` Now a direct beachhead signal, not just an adjacent hint: one open answer asks for "a calculator: how much to contribute per month to reach the desired result" `[N=30, R26]`, alongside FIREkit's FIRE projection (§1). A related request asks for "statistical interpolation for the past and the future" `[N=30, R23]` - an analytics, not a goal, want. Both are single voices (R26's own top difficulty is "I'm fine"), so this stays thin, but it lifts HJ3 from no data to one direct request and motivates the complementary Goals/Analytics screens.
- **HJ4 (P2) - Stay aware without maintaining anything.** *When I check my money, I want a glance rather than a system to keep up, so I can stay aware with near-zero effort.* `[?]` Inferred from the 20% who consolidate nowhere `[N=30, Q4]`; their actual motivation is unknown.
